...
namespace quat {
    ...
    export function rotateX(q: Fx8[], angleTwos: Fx8) {
        const halfAngle = Fx.rightShift(angleTwos, 1)
        const s = Fx.sinFx8(halfAngle)
        const c = Fx.cosFx8(halfAngle)
        const x = q[0]
        const y = q[1]
        const z = q[2]
        const w = q[3]
    ...
    export function rotateY(q: Fx8[], angleTwos: Fx8) {
        const halfAngle = Fx.rightShift(angleTwos, 1)
        const s = Fx.sinFx8(halfAngle)
        const c = Fx.cosFx8(halfAngle)
        const x = q[0]
    ...
    export function rotateZ(q: Fx8[], angleTwos: Fx8) {
        const halfAngle = Fx.rightShift(angleTwos, 1)
        const s = Fx.sinFx8(halfAngle)
        const c = Fx.cosFx8(halfAngle)
        const x = q[0]
...
function scale_by_fraction(x: number, a: number, b: number) {
    ...
    // FIXME: this doesn't work correctly for large numbers?
    //return Fx.toIntFloor(Fx.idiv(Fx.imul(Fx8(x), a), b))

    const ival = Math.idiv(Math.imul(x | 0, a | 0), b | 0)
    //const fval = Math.floor(x * a / b)
    //console.log("i=" + ival + " f=" + fval + " x=" + x + " a=" + a + " b=" + b)
    return ival
}
...
class MeshModelBase {
    ...
    calculateBoundingSphere() {
        ...
        this.boundingSphereRadius = Math.round(r)
    }
        ...
        */
        ...
        for (let i = 0; i < clippedFaces.length; ++i) {
            ...
            if (np == 3) {
                ...
                // from the camera).
                if (!Polygon.isCounterclockwise(p0, p1, p2)) {
                    if (this.isTwoSided) {
                        const tmp = p0
                        p0 = p2
                        p2 = tmp
                ...
                if (np >= 6) {
                    ...
                    if (!Polygon.isCounterclockwise(points[i0], points[i1], points[i2])) {
                        if (this.isTwoSided) {
                            for (let i = 0; i < np / 2; ++i) {
                                const tmp = points[np - i - 1]
                                points[np - i - 1] = points[i]
                                points[i] = tmp
                            }
                        } else {
                            continue
                    ...
                    if (!Polygon.isCounterclockwise(points[0], points[1], points[2])) {
                        if (this.isTwoSided) {
                            for (let i = 0; i < np / 2; ++i) {
                                const tmp = points[np - i - 1]
                                points[np - i - 1] = points[i]
